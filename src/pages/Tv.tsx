...
export default function Tv() {
  ...
  useEffect(() => {
    const fetchTv = async () => {
      try {
        if (querySearch) {
          const { data } = await api.get<{ results: APITvResponse[] }>("/search/tv", {
            params: {
              query: querySearch,
              page: 1,
            },
  ...
  return (
    <>
      <div className="container mx-auto my-8 py-8">
        <MovieSection
          title={querySearch ? `Search for "${querySearch}"` : "Trending Tv"}
          movies={tv}
          isLoading={isLoading}
        />
        {!tv.length && <h1 className="text-center text-2xl font-bold">No results found</h1>}
